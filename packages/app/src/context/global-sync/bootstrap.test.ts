...
import type { AgentApi, CatalogApi, CommandApi, ReferenceApi, SkillApi } from "@opencode-ai/client/promise"
...
const api = {
  ...
  skill: { list: async () => ({ location: {}, data: [] }) },
...
describe("bootstrapDirectory", () => {
  test("uses legacy MCP endpoints while refreshing a v1 directory", async () => {
    ...
    await bootstrapDirectory({
      ...
      sdk: {
        app: {
          agents: async () => ({ data: [{ name: "build", mode: "primary" }] }),
          skills: async () => {
            mcpReads.push("skill")
            return { data: [] }
          },
    ...
    expect(mcpReads.sort()).toEqual(["command", "resource", "skill", "status"])
  })
})
...
describe("query keys", () => {
  ...
  test("excludes legacy skill commands from the slash command list", async () => {
    const legacy = {
      command: {
        list: async () => ({
          data: [
            { name: "review", template: "Review files", source: "command" },
            { name: "planner", template: "# Planner", source: "skill" },
          ],
        }),
      },
    } as unknown as OpencodeClient
  ...
  test("loads skills from the current location-scoped endpoint", async () => {
          ...
          data: [
            {
              ...
              location: "/skills/planner/SKILL.md",
              content: "# Planner",
            },
          ],
        }
    ...
    const result = await loadSkills("/repo", api)
