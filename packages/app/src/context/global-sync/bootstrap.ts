...
export const loadCommands = (
  ...
  retry(async () => {
    if ((await protocol) === "v1" && legacy) {
      return ((await legacy.command.list()).data ?? [])
...
export const loadSkills = (
  ...
  retry(async () => {
    if ((await protocol) === "v1" && legacy) {
      return ((await legacy.app.skills({ directory })).data ?? []).map((skill) => ({
        id: skill.name,
        name: skill.name,
    ...
    return api
      ...
      .then((result) => result.data.map((skill) => ({ ...skill, id: skill.id || skill.name })))
  })
...
export async function bootstrapDirectory(input: {
  ...
  ;(async () => {
    const slow = [
      ...
      input.mcp &&
        (() =>
          loadSkills(input.directory, input.api.skill, input.sdk, input.protocol).then((skills) =>
            input.setStore("skill", skills),
          )),
      () =>
        input.queryClient.fetchQuery(
