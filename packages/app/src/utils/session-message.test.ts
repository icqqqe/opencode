...
describe("normalizeSessionMessages", () => {
  ...
  test("restores persisted prompt text parts instead of rendering expanded V2 prompt text", () => {
    const skill = {
      name: "planner",
    ...
    const textParts: JsonValue[] = [
      ...
      { synthetic: true, metadata: { opencodeSkill: skill } },
    ]
    const source = [
      {
        id: "msg_skill",
        type: "user",
        text: 'use $planner then $planner\n<skill_content name="planner">expanded</skill_content>',
        metadata: {
          opencodePrompt: {
            textParts,
          },
    ...
    const result = normalizeSessionMessages("ses_1", source)
    const parts = result.parts.get("msg_skill") ?? []
    ...
    expect(parts).toEqual([
      ...
      expect.objectContaining({
        ...
        text: "",
        synthetic: true,
        metadata: { opencodeSkill: skill },
      }),
    ])
    expect(extractPromptFromParts(parts).filter((part) => part.type === "skill")).toEqual([
      expect.objectContaining({ name: "planner", location: skill.location, start: 4, end: 12 }),
      expect.objectContaining({ name: "planner", location: skill.location, start: 18, end: 26 }),
    ])
  })
