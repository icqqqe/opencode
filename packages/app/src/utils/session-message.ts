...
function persistedPromptTextParts(metadata: unknown): PersistedPromptTextPart[] | undefined {
  if (!record(metadata)) return
  const prompt = metadata.opencodePrompt
  if (!record(prompt) || !Array.isArray(prompt.textParts)) return
  const parts = prompt.textParts.flatMap((part): PersistedPromptTextPart[] => {
    if (!record(part)) return []
    if (part.text !== undefined && typeof part.text !== "string") return []
    if (part.synthetic !== undefined && typeof part.synthetic !== "boolean") return []
    if (part.ignored !== undefined && typeof part.ignored !== "boolean") return []
    if (part.metadata !== undefined && !record(part.metadata)) return []
    if (part.text === undefined && (part.synthetic !== true || !part.metadata)) return []
    return [
      {
        text: part.text ?? "",
  ...
  if (parts.length !== prompt.textParts.length) return
  return parts
}
...
function userParts(sessionID: string, message: SessionMessageUser): Part[] {
  const textParts: PersistedPromptTextPart[] = persistedPromptTextParts(message.metadata) ?? [{ text: message.text }]
  return [
    ...textParts.map((part, index) => ({
      ...textPart(sessionID, message.id, index, part.text, part.synthetic),
      ignored: part.ignored,
      metadata: part.metadata,
    })),
    ...(message.files ?? []).map(
