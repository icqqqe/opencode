...
const instructionLayer = (input: {
  ...
  AppNodeBuilder.build(LayerNode.group([Config.node, SystemContextRegistry.node, InstructionContext.node]), [
    [
      Config.node,
      Layer.succeed(
        Config.Service,
        Config.Service.of({ entries: () => Effect.succeed(input.configEntries ?? []) }),
      ),
    ],
...
describe("InstructionContext", () => {
  ...
  it.live("skips automatic AGENTS.md when instruction autodiscovery is disabled", () =>
    Effect.acquireRelease(
      Effect.promise(() => tmpdir()),
      (tmp) => Effect.promise(() => tmp[Symbol.asyncDispose]()),
    ).pipe(
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          const file = path.join(tmp.path, "AGENTS.md")
          yield* Effect.promise(() => fs.writeFile(file, "hidden"))
          const context = yield* SystemContextRegistry.Service.pipe(
            Effect.flatMap((service) => service.load()),
            Effect.provide(
              instructionLayer({
                ...
                configEntries: [
                  new Config.Document({
                    type: "document",
                    info: new Config.Info({ autodiscover_instructions: false }),
                  }),
                ],
                locationServiceLayer: Layer.succeed(
                  Location.Service,
